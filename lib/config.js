const args_parser = require('./args');
const path = require('path');
const deepextend = require('deep-extend');
const debug = require('debug')('chen-config');

let config_loaded = false;
let config_object = {};

const objectGet = (obj, prop, delimiter = '.') => {
    if(!obj){
        return null;
    }

    if(!prop){
        return obj;
    }

    if(prop.indexOf(delimiter) < 0){
        return obj[prop];
    }

    let value = obj;
    for(let i of prop.split(delimiter)){
        if(value.hasOwnProperty(i)){
            value = value[i];
        }
        else{
            return null;
        }
    }

    return value;
};

const safeRequirePath = (file_path, forceReload = false, silent = false) => {
    if(!path.isAbsolute(file_path)){
        file_path = path.join(process.cwd(), file_path);
    }
    try{
        if(forceReload){
            delete require.cache[require.resolve(file_path)];
        }
        return require(file_path)
    }
    catch(e){
        if(!silent){
            debug("chen.js: could not load file", file_path, JSON.stringify(e));
        }
        return {};
    }
};

const objectSet = (obj, prop, set_value, delimiter = '.') => {
    if(!obj || !prop){
        return;
    }

    if(prop.indexOf(delimiter) < 0){
        return obj[prop] = set_value;
    }

    let value = obj;
    for(let i of prop.split(delimiter).slice(0, -1)){
        if(!value.hasOwnProperty(i)){
            value[i] = {};
        }
        value = value[i];
    }

    return value[prop.split(delimiter).pop()] = set_value;
};

const props = {
    load: (args = args_parser(), local_config_path = './config/default', default_config_path = ['./config/default']) => {
        if(!Array.isArray(default_config_path)){
            default_config_path = [default_config_path];
        }

        config_object = {};

        config_object = deepextend(
            {},
            ...default_config_path.map(file => safeRequirePath(file, true)),
            safeRequirePath(args.config || local_config_path, true)
        );

        for(let i in args){

            if(i === 'config') continue;
            if(i.indexOf('config.') === 0){
                objectSet(config_object, i.slice('config.'.length), args[i]);
            }
            else{
                objectSet(config_object, `args.${i}`, args[i]);
            }
        }
        config_loaded = true;

        return module.exports;
    },

    _load: (...args) => props.load(undefined, ...args),

    get: (prop) => {
    	if(!config_loaded){
    		module.exports.load();
    	}
    	return objectGet(config_object, prop)
    },

    resolve: () => {
    	if(!config_loaded){
    		module.exports.load();
    	}
    	return Object.assign({}, config_object)
    }
};

module.exports = new Proxy(props._load, {
    get: (target, prop) => props[prop],
    set: () => undefined
});
